...
export class TournamentGamePage extends Component {
    ...
    new tournamentTree($tournamentTree, {});
		const $resultTarget = this.$target.querySelector('[data-component="tournamentGameResult"]') as HTMLElement;

		this.gameStartSetting($waitingModal);
		this.tournamentLoop($waitingModal, $tournamentTree, $resultTarget);
	}

	tournamentLoop(waitingModal: HTMLElement, tournamentTree: HTMLElement, resultTarget: HTMLElement) {
		this.socket.setMessage();
		// this.socket.msg = msgList[idx]; // test용
		// idx = idx + 1; // test용
		
		if (this.socket.msg.type === "connection")
			this.checkConnection(this.socket.msg);

		setTimeout((socket: TournamentSocket) => {
			this.socket = socket;
