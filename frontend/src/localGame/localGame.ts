...
export function startLocalGame(canvas: HTMLCanvasElement) {
  ...
  function MoveBallLoop(ball: Ball) {
    if (gameStatus.WKeyState && gameWindow.Pad1YPos > 0) gameWindow.Pad1YPos -= 10;
    if (gameStatus.SKeyState && gameWindow.Pad1YPos < window.innerHeight - 100)
      gameWindow.Pad1YPos += 10;
    if (gameStatus.OKeyState && gameWindow.Pad2YPos > 0) gameWindow.Pad2YPos -= 10;
    ...
    if (gameStatus.RequestFrame) requestAnimationFrame(() => MoveBallLoop(ball));
  }
}
