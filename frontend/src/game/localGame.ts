...
let gameWindow: GameWindow = new GameWindow();
...
document.addEventListener('keydown', (e) => {
  ...
  if (e.key == "Enter") {
    if (!gameStatus.RequestFrame) {
      var ball = new Ball(gameWindow.DocWidth / 2, gameWindow.DocHeight / 2, 30);
...
//window.onresize = gameWindow.drawCanvas;

gameWindow.canvas.onclick = () => {
  if (!gameStatus.RequestFrame) {
    var ball: Ball = new Ball(gameWindow.DocWidth / 2, gameWindow.DocHeight / 2, 30);
    ball.drawObject(gameWindow.ctx);
    gameStatus.RequestFrame = true;
    MoveBallLoop(ball);
  }
